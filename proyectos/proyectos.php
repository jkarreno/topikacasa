...
if(isset($_POST["hacer"]))
{
    if($_POST["hacer"] == "addproyecto")
    {
        ...
        $sql = "INSERT INTO proyectos (IdCliente, NombreProyecto, FechaCreacion) 
                                    VALUES ('".$cliente."', '".$nombre_proyecto."', '".time()."')";
...
$cadena=$mensaje.'<div class="c100 card agc ber bff bfz">
            ...
            <table id="table_proyectos" class="stripe row-border order-column nowrap">
                ...
                <tbody>';
$ResProyectos = mysqli_query($conn, "SELECT p.Id, p.NombreProyecto, p.FechaCreacion, p.FechaInicio, p.FechaFin, p.Estatus, c.Nombre AS NombreCliente 
                                        FROM proyectos p 
...
while ($RResProyectos = mysqli_fetch_assoc($ResProyectos)) {
    $cadena.='          <tr>
                            ...
                            <td>'.$RResProyectos["NombreProyecto"].'</td>
                            <td>'.$RResProyectos["NombreCliente"].'</td>
                            <td>'.fecha(date("Y-m-d", $RResProyectos["FechaCreacion"])).'</td>
                            <td>'.($RResProyectos["FechaInicio"]!=NULL ? fecha(date("Y-m-d", $RResProyectos["FechaInicio"])) : '---').'</td>
                            <td>'.($RResProyectos["FechaFin"]!=NULL ? fecha(date("Y-m-d", $RResProyectos["FechaFin"])) : '---').'</td>
...
function editar_proyecto(idproyecto){
    ...
    $.ajax({
				type: 'POST',
				url : 'proyectos/editar_proyecto.php',
